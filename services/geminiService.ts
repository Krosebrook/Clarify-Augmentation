...
import { AnalysisResult, GeneratedImage, ChatMessage } from "../types";
...
- "mini": Use for small, less critical details (label + icon only).

Provide this data:
- "label": Name (1-4 words)
- "format": "compact" | "stats" | "detailed" | "mini"
- "description": Rich text (approx 30-50 words).
- "category": "concept" | "data" | "process" | "structure" | "highlight"
...
Mix formats. Use "highlight" category for the most important element. Ensure the "bounds" accurately target specific visual elements in the image.
...
export const askAboutSegment = async (
  question: string,
  imageBase64: string,
  segmentContext: { label: string; description: string },
  history: ChatMessage[]
): Promise<string> => {
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  ...
  const systemPrompt = `
You are an expert tutor answering questions about a specific part of an image.
The user is looking at a segment labeled "${segmentContext.label}".
Description of this segment: "${segmentContext.description}".

Your goal is to answer the user's question specifically about this visual element.
Keep your answer concise (under 3 sentences) and conversational.
If the question is unrelated to the visual, gently bring it back to the image context.
`;
