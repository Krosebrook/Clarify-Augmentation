...
function App() {
  ...
  // Store the query used for generation alongside the data to ensure consistency in downloads
  const [data, setData] = useState<{ image: GeneratedImage; analysis: AnalysisResult | null; executedQuery: string } | null>(null);
  ...
  const processSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      setData({ image, analysis: null, executedQuery: searchQuery });
      setStatus('analyzing');
  ...
  const handleDownload = () => {
    ...
    // Use the executedQuery for the filename, falling back to 'image' if missing
    const filenameQuery = (data.executedQuery || 'lumina-image').toLowerCase().replace(/\s+/g, '-');
    link.download = `lumina-${filenameQuery}-${Date.now()}.png`;
  ...
  return (
    <div className="h-screen w-screen bg-[#050505] text-white selection:bg-cyan-500/30 selection:text-cyan-200 overflow-hidden flex flex-col relative">
      
      {/* Dynamic Background Layer */}
      <HeroVideo 
        mp4Src="/lumina-bg.mp4" 
        posterSrc=""
              ...
              >
                 {status === 'complete' && (
                   <div className="text-center mb-2 flex-none z-20">
                      <h2 className="text-xl font-bold text-white capitalize shadow-black drop-shadow-md">{data.executedQuery}</h2>
                   </div>
                 )}
